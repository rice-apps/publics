  ...
  return (
    <div className="mx-auto mx-4 space-y-4">
      <div className="flex justify-between mt-4">
        <h1>{shifts[0].event.name}: Shifts</h1>
        <AddShiftModal eventId={shifts[0].event.id}/>
      </div>
      {/* create a grid with 3 columns of shiftcards */}
        <div key="shifts" className="grid md:grid-cols-3 gap-4">
        {shifts.filter((shift) => shift.name == "Capacity Counter").length > 0 && (
            <div className="md:col-span-3">
            <h2>Capacity Counter</h2>
          </div>
            )}
          {shifts.filter((shift) => shift.name == "Capacity Counter").map((shift) => (
            <ShiftCard key={shift.id} id={shift.id} start={shift.start} end={shift.end} count={shift.checkedIn} total={shift.count} />
          ))}
          ...
          {shifts.filter((shift) => shift.name == "Security").map((shift) => (
            <ShiftCard key={shift.id} id={shift.id} start={shift.start} end={shift.end} count={shift.checkedIn} total={shift.count} />
          ))}
          {shifts.filter((shift) => shift.name == "Caregiver").length > 0 && (
            ...
            {shifts.filter((shift) => shift.name == "Caregiver").map((shift) => (
            <ShiftCard key={shift.id} id={shift.id} start={shift.start} end={shift.end} count={shift.checkedIn} total={shift.count} />
            ))}
...
export const getServerSideProps = async (ctx) => {
  ...
  if (error || shifts.length === 0 || volunteersError || volunteers.length === 0) {
      return {
          redirect: {
  ...
  shifts.forEach((shift) => {
    shift.count = volunteers.filter((volunteer) => volunteer.shift === shift.id)
      .length
  })
